...
|---|---|
| 2026-08-21-09-51 | **Removed the dedicated periodic-CDR timer added 2026-08-19-15-00, by request — traced to a hard-fault crash**: a Memfault hard-fault trace (issue 1805272539, trace 10680362446) showed the system workqueue (`sysworkq`) stack-overflowing right after an nRF70 FW stats collection log line; this project's `prj.conf` had set the periodic-CDR interval to 60 s, so that deep Wi-Fi-firmware-stats call ran on the shared, tightly-sized system workqueue every minute. Same-day fix: nRF70 FW stats are now collected once per Memfault metrics heartbeat (same cadence as metrics/log collection) instead of on a separate dedicated timer — no behavior change users would notice (a fresh CDR snapshot is still kept ready for upload), no requirement change, purely an internal reliability fix. See [app-memfault-module.md](../dev-specs/app-memfault-module.md) Changelog for the full account, including a still-open note that this alone doesn't shrink the stack footprint of the underlying firmware-stats call. |
...
- This `ncs-v2.6.4` branch targets **NCS v2.6.4**, which uses legacy Partition Manager (`pm_static_<board>.yml`) — not the DTS-based partitioning used on NCS v3.3+. Board targets use the legacy underscore format (`nrf7002dk_nrf5340_cpuapp`) and require the explicit `--sysbuild` flag.
