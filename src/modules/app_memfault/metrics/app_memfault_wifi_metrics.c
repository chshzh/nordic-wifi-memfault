...
void mflt_wifi_metrics_collect(void)
{
	...
	const char *band_str = status.band == WIFI_FREQ_BAND_2_4_GHZ ? "2.4"
	...
	/* Heartbeat fallback for wifi_ap_ssid/wifi_ap_bssid: catches roams that don't
	 * produce a fresh (non-deduped) DHCP-bound event, e.g. a reassociation
	 * that keeps the same IP. Re-setting an unchanged value is a no-op on
	 * the Memfault side (latest-value-wins), and the change counters only
	 * increment on an actual diff, so this is safe to call every heartbeat.
	 */
	...
	LOG_DBG("WiFi metrics collected: RSSI=%d dBm, Channel=%u, TX rate=%.1f Mbps, OUI=%s",
