...
static struct memfault_ncs_metrics_thread stack_metrics_threads[] = {
	...
	{.thread_name = "memfault_upload_tid",
...
static bool stack_metrics_threads_registered[ARRAY_SIZE(stack_metrics_threads)];
...
void mflt_stack_metrics_init(void)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(stack_metrics_threads); i++) {
		if (stack_metrics_threads_registered[i]) {
			registered_count++;
			continue;
		}
		...
		if (err) {
			...
			stack_metrics_threads_registered[i] = true;
			registered_count++;
	...
	LOG_INF("Stack metrics monitoring active for %zu/%zu threads", registered_count,
		ARRAY_SIZE(stack_metrics_threads));
}
#endif
